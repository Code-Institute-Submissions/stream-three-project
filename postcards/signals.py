...
def subscription_created(sender, **kwargs):
    ipn_obj = sender
    postcard_id = ipn_obj.custom.split('-')[0]
    user_id = ipn_obj.custom.split('-')[1]
    Purchase.objects.create(postcard_id=postcard_id,
                            user_id=user_id,
...
def subscription_was_cancelled(sender, **kwargs):
    ...
    postcard_id = ipn_obj.custom.split('-')[0]
    user_id = ipn_obj.custom.split('-')[1]
    purchase = Purchase.objects.get(user_id=user_id, postcard_id=postcard_id)
